...
<template>
  <div class="app-container">
    <!-- 搜索栏 -->
    <el-form @submit.native.prevent :model="logQuery" :inline="true" v-show="showSearch">
      <el-form-item label="访问页面">
        <el-input @keyup.enter="queryVisitLogs" v-model="logQuery.keywords" style="width: 200px"
                  placeholder="请输入访问页面" clearable />
      </el-form-item>
      <el-form-item>
        <el-button type="primary" icon="Search" @click="queryVisitLogs">搜索</el-button>
      </el-form-item>
    </el-form>
    <!-- 操作按钮 -->
    <el-row :gutter="10" class="mb8">
      ...
      <right-toolbar v-model:showSearch="showSearch" @queryTable="getVisitLogList"></right-toolbar>
    </el-row>
    <!-- 表格展示 -->
    ...
    <pagination v-if="count > 0" :total="count" v-model:page="logQuery.current" v-model:limit="logQuery.size"
                @pagination="getVisitLogList" />
  </div>
</template>

<script setup lang="ts">
import logApi from '@/api/log';
import type { LogQuery, VisitLog } from '@/api/log/types';
...
const getVisitLogList = () => {
  loading.value = true;
  logApi.getVisitLogList(logQuery.value).then(({ data }) => {
