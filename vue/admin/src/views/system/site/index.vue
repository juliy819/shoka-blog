      ...
      <el-tab-pane>
        ...
        <el-form label-width="80px" :model="siteConfig" label-position="left">
          <el-row>
            <el-col :md="6">
              <el-form-item label="用户头像">
                <el-upload class="avatar-uploader" :headers="authorization" action="/api/site/upload/user"
                           :show-file-list="false" accept="image/*" :before-upload="beforeUpload"
                           :on-success="handleUserAvatarSuccess">
                  <img v-if="siteConfig.userAvatar" :src="siteConfig.userAvatar" class="avatar" alt="" />
            ...
            <el-col :md="6">
              <el-form-item label="游客头像">
                <el-upload class="avatar-uploader" :headers="authorization" action="/api/site/upload/tourist"
                           :show-file-list="false" accept="image/*" :before-upload="beforeUpload"
                           :on-success="handleTouristAvatarSuccess">
                  <img v-if="siteConfig.touristAvatar" :src="siteConfig.touristAvatar" class="avatar" alt="" />
                  <el-icon v-else class="avatar-uploader-icon">
                    <Plus />
                  </el-icon>
                </el-upload>
          ...
          <el-form-item label="网站公告">
            <el-input style="width: 400px;" v-model="siteConfig.siteNotice" :autosize="{ minRows: 4, maxRows: 5 }"
          ...
          <el-form-item>
            <el-button type="primary" @click="updateSiteConfig">保 存</el-button>
          </el-form-item>
        </el-form>
      </el-tab-pane>
      <!-- 作者信息 -->
      <el-tab-pane label="author">
        ...
        <el-form label-width="80px" :model="siteConfig" label-position="left">
          <el-form-item label="作者头像">
            <el-upload class="avatar-uploader" :headers="authorization" action="/api/site/upload/author"
                       :show-file-list="false" accept="image/*" :before-upload="beforeUpload"
                       :on-success="handleAuthorAvatarSuccess">
              <img v-if="siteConfig.authorAvatar" :src="siteConfig.authorAvatar" class="avatar" alt="" />
              <el-icon v-else class="avatar-uploader-icon">
                <Plus />
              </el-icon>
            </el-upload>
          ...
          <el-form-item label="关于我">
            <v-md-editor v-model="siteConfig.aboutMe" :disabled-menus="[]" :left-toolbar="toolList"
                         @upload-image="uploadSiteImage" height="400px" />
          </el-form-item>
          <el-form-item>
            <el-button type="primary" @click="updateSiteConfig">保 存</el-button>
          </el-form-item>
      ...
      <el-tab-pane label="other">
        ...
        <el-form label-width="100px" :model="siteConfig" label-position="left">
          <el-form-item label="文章默认封面">
            <el-upload class="avatar-uploader" :headers="authorization" action="/api/site/upload/article"
                       :show-file-list="false" accept="image/*" :before-upload="beforeUpload"
                       :on-success="handleArticleSuccess">
              <img v-if="siteConfig.articleCover" :src="siteConfig.articleCover" class="article-cover" alt="" />
              <el-icon v-else class="avatar-uploader-icon">
                <plus />
              </el-icon>
            </el-upload>
...
const uploadSiteImage = (event: any, insertImage: any, files: File[]) => {
  files.forEach(file => {
    let formData = new FormData();
    formData.append('file', file);
    siteApi.uploadSiteImg(formData).then(({ data }) => {
      insertImage({
        url: data.data
      });
    });
...
const updateSiteConfig = () => {
  ...
  siteApi.updateSiteConfig(siteConfig.value).then(() => {
    modal.notifySuccess('更新成功');
    getSiteConfig();
  });
};
const getSiteConfig = () => {
  siteApi.getSiteConfig().then(({ data }) => {
